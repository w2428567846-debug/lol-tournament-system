...
export function TournamentRegistrationForm({
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    router.replace(`/account?${editing ? 'updated' : 'registered'}=${encodeURIComponent(tournament.slug)}`);
    router.refresh();
  }
  ...
  return (
    <form onSubmit={submit} className="mt-7 grid gap-5 sm:grid-cols-2">
      ...
      {editing && registration.status !== 'PENDING' ? <p className="border border-amber-300/20 bg-amber-300/6 px-4 py-3 text-xs leading-5 text-amber-100 sm:col-span-2">修改游戏 ID、段位或位置后，这份报名会自动回到“等待审核”。</p> : null}
      ...
      <div className="sm:col-span-2"><button disabled={submitting} className="gold-button min-h-12 px-8 text-sm font-black tracking-[.12em] text-[#080b10] disabled:opacity-60">{submitting ? '保存中…' : editing ? '保存报名修改' : '确认提交报名'}</button></div>
    </form>
  );
}
