              ...
              {registrations.map((registration) => (
                <article key={registration.id} className="grid min-w-0 gap-5 border border-white/9 bg-[#0d1219] p-5 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center sm:p-6">
                  <div className="min-w-0">
                    ...
                    {registration.status === 'APPROVED' ? <p className="mt-2 text-xs text-slate-500">赛事数据：{registration.teamName ?? '尚未分队'} · {registration.valuation == null ? '费用待定' : `${registration.valuation.toFixed(1)} 费`}{registration.matchesPlayed > 0 ? ` · ${registration.matchesPlayed} 场 ${registration.wins} 胜 ${registration.losses} 负 · ${registration.kills}/${registration.deaths}/${registration.assists}` : ''}{registration.placement ? ` · 第 ${registration.placement} 名` : ''}</p> : null}
