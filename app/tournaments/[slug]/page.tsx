    ...
    <main className="min-h-screen bg-[#080b10] text-white">
      ...
      <section className="relative overflow-hidden border-b border-white/8 bg-[#0a0e14]">
        ...
        <div className="relative mx-auto max-w-7xl px-5 py-14 sm:px-8 sm:py-20 lg:px-10">
          ...
          <h1 className="mt-5 max-w-4xl [overflow-wrap:anywhere] text-4xl font-black tracking-[-.05em] sm:text-6xl">{tournament.name}</h1>
      ...
      <div className="mx-auto grid max-w-7xl gap-10 px-5 py-12 sm:px-8 lg:grid-cols-[1fr_360px] lg:px-10">
        <div className="space-y-10">
          ...
          <section><SectionTitle eyebrow="Participants" title="参与者预览" />{tournament.participantsRestricted ? <div className="mt-5 border border-dashed border-white/12 p-7 text-sm leading-6 text-slate-500">这是群内赛事。只有管理员或仍在本赛事有效报名名单中的玩家可以查看选手资料。报名人数仍会正常显示。</div> : tournament.participants.length === 0 ? <div className="mt-5 border border-dashed border-white/12 p-7 text-sm text-slate-600">暂未公布已通过选手。</div> : <div className="mt-5 grid gap-3 sm:grid-cols-2">{tournament.participants.map((participant) => <article key={`${participant.gameId}-${participant.primaryRole}`} className="min-w-0 border border-white/9 bg-[#0d1219] p-5"><div className="flex items-start justify-between gap-4"><div className="min-w-0"><p className="[overflow-wrap:anywhere] font-black text-white">{participant.gameId}</p><p className="mt-2 text-xs text-slate-500">{playerRoleLabels[participant.primaryRole]} · {participant.rank}</p><p className="mt-1 text-xs text-slate-600">{participant.teamName ?? '待分队'}</p></div><span className="shrink-0 text-sm font-black text-[#d8b968]">{participant.valuation == null ? '费用待定' : `${participant.valuation.toFixed(1)} 费`}</span></div>{participant.matchesPlayed > 0 ? <p className="mt-4 border-t border-white/8 pt-3 text-xs text-slate-500">{participant.matchesPlayed} 场 · {participant.wins} 胜 {participant.losses} 负 · {participant.kills}/{participant.deaths}/{participant.assists}{participant.placement ? ` · 第 ${participant.placement} 名` : ''}</p> : null}</article>)}</div>}</section>
        </div>
        <aside>
          <div className="border border-[#d8b968]/25 bg-[linear-gradient(145deg,rgba(216,185,104,.12),rgba(13,18,25,.98)_52%)] p-6 lg:sticky lg:top-6">
