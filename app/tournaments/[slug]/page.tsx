    ...
    <main className="min-h-screen bg-[#080b10] text-white">
      ...
      <div className="mx-auto grid max-w-7xl gap-10 px-5 py-12 sm:px-8 lg:grid-cols-[1fr_360px] lg:px-10">
        <div className="space-y-10">
          ...
          <section><SectionTitle eyebrow="Participants" title="参与者预览" />{tournament.participantsRestricted ? <div className="mt-5 border border-dashed border-white/12 p-7 text-sm leading-6 text-slate-500">这是群内赛事。只有管理员或仍在本赛事有效报名名单中的玩家可以查看选手资料。报名人数仍会正常显示。</div> : tournament.participants.length === 0 ? <div className="mt-5 border border-dashed border-white/12 p-7 text-sm text-slate-600">暂未公布已通过选手。</div> : <div className="mt-5 grid gap-3 sm:grid-cols-2">{tournament.participants.map((participant) => <article key={`${participant.gameId}-${participant.primaryRole}`} className="min-w-0 border border-white/9 bg-[#0d1219] p-5"><p className="[overflow-wrap:anywhere] font-black text-white">{participant.gameId}</p><p className="mt-2 text-xs text-slate-500">{playerRoleLabels[participant.primaryRole]} · {participant.rank}</p></article>)}</div>}</section>
        </div>
        <aside>
