...
- Migrations 001 through 009 plus disposable PostgreSQL integration tests.
...
202608250009_split_public_tournament_read_policy.sql
```
...
The final `migration list` must show all nine timestamps in both LOCAL and REMOTE. Never run `db reset --linked` against production and never use `--include-seed` for production.
...
The production path is the same fresh-chain path used by `pnpm test:integration`: PostgreSQL 17, Supabase-style `anon`, `authenticated`, and `service_role` defaults, migrations sorted by filename from 001 through 009, then schema and behavior verification. Historical migrations remain unchanged.
