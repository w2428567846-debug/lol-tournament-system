...
This audit covers migrations 001 through 009 and the effective schema after they run in filename order.
...
- Migration 009 separates anonymous public tournament reads from the authenticated-only `is_admin()` policy so anonymous REST queries never require permission to execute an account helper.
...
- The independent integration CI job uses a disposable PostgreSQL 17 service and emulates Supabase's default API-role grants. A passing run proves the 001-to-009 chain and the final catalog/behavior checks, not compatibility with an already-modified production database.
